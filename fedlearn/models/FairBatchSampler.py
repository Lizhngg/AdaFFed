...
class FairBatch(Sampler):
    ...
    def __init__(self, model, x_tensor, y_tensor, z_tensor, batch_size, alpha, target_fairness, replacement = False, seed = 0):
        ...
        print(x_tensor.shape, y_tensor.shape, z_tensor.shape)
        self.x_data = x_tensor
        self.y_data = y_tensor
        self.z_data = z_tensor
        
        print(self.x_data.shape, self.y_data.shape, self.z_data.shape)
    ...
    def adjust_lambda(self):
        ...
        if self.fairness_type == 'eqopp':
            ...
            eo_loss = criterion ((F.tanh(logit)+1)/2, ((self.y_data+1)/2).reshape(-1,1))
            ...
            dp_loss = criterion((F.tanh(logit)+1)/2, ones_tensor.reshape(-1,1)) # Note that ones tensor puts as the true label
